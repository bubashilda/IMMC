...
class model(Frame):
    ...
    def get_graphics(self):
        ...
        for i in range(0, len(main_road)):
            ...
            canvas.create_line(main_road[i][1] * k, 0, (main_road[i][1] + 1) * k, 0, width = 10)

        for i in range(0, len(list_of_roads)):
            for j in range(0, len(list_of_roads[i])):
                canvas.create_line((list_of_moving_points[i] + n) * k, ((list_of_roads[i][j][1])) * k + n * k, (list_of_moving_points[i] + road_wight - n) * k, ((list_of_roads[i][j][1])) * k + n * k, width = 4)

        for i in list_of_seats:
            if road_wight * (3 - i[2]) > 0:
                help_1 = road_wight * (3 - i[2]) + road_wight // 2
            else:
                help_1 = road_wight * (3 - i[2]) - road_wight // 2

            canvas.create_rectangle((list_of_moving_points[i[0]] + help_1) * k, (i[1] + 2) * k, ((list_of_moving_points[i[0]] + help_1) + road_wight) * k, (i[1] + 3) * k)
